...
var rk11 = {
    rkds: 0o4700, // 0o17777400 Drive Status
    rker: 0, // 0o17777402 Error Register
    rkcs: 0o200, // 0o17777404 Control Status
    rkwc: 0, // 0o17777406 Word Count
    rkba: 0, // 0o17777410 Bus Address
    rkda: 0, // 0o17777412 Disk Address
    meta: [],
...
function rk11_finish(drive) {
    if (rk11.rkcs & 0x40) {
        interrupt(0, 10, 5 << 5, 0o220, rk11_commandEnd, drive);
    ...
    if (rk11.TRACKS[drive] == 0) {
        ...
        switch ((rk11.rkcs >> 1) & 7) { // function code
            ...
            case 4: // Seek - complete immediately
                ...
                interrupt(0, 20, 5 << 5, 0o220, rk11_seekEnd, drive);
                break;
            case 5: // Read Check
                break;
            case 6: // Drive Reset
                rk11.rkds = 0o4700 | (drive << 13);
                rk11.rker = 0; //
                rk11.rkcs = 0o200;
...
var rl11 = {
    csr: 0x81, // 0o17774400 Control status register
    bar: 0, // 0o17774402 Bus address
    dar: 0, // 0o17774404 Disk address
    mpr: 0, // 0o17774406 Multi purpose
...
function rl11_finish(drive?) { // Todo optional?
    if (rl11.csr & 0x40) {
        interrupt(0, 10, 5 << 5, 0o160, rl11_commandEnd);
    } else { // if interrupt not enabled just mark completed
        rl11_commandEnd();
    }
}
...
var rp11 = {
    rpas: undefined, // Todo: this was missing
    DTYPE: [0o20022, 0o20022, 0o20020, 0o20020, 0o20022, 0o20020, 0o20022, 0o20042], // Drive type rp06, rp06, rp04, rp04...
...
function rp11_go() {
    ...
    interrupt(1, 12, 5 << 5, 0o254, function() {
...
function rp11_end(err, meta, position, address, count) {
    ...
    interrupt(1, 20, 5 << 5, 0o254, function() {
        ...
        case 0o17776700: // rp11.rpcs1 Control status 1
            ...
            if (data >= 0) {
                ...
                if (result >= 0) {
                    ...
                    if (!(result & 0x40)) interrupt(1, -1, 0, 0o254); //remove pending interrupt if IE not set
                    if ((data & 0xc0) == 0xc0) interrupt(1, 8, 5 << 5, 0o254); // RB:
                    rp11.rpcs1 = result;
...
function tm11_finish() {
    if (tm11.mtc & 0x40) {
        interrupt(0, 10, 5 << 5, 0o224, tm11_commandEnd);
    } else { // if interrupt not enabled just mark completed
        tm11_commandEnd();
    }
}
...
function tty_rbuf() {
    if (!(tty.rcsr & 0x80)) {
        if (tty.rbufQueue.length > 0) {
            ...
            tty.rcsr |= 0x80;
            if (tty.rcsr & 0x40) interrupt(1, 0, 4 << 5, 0o60);
        }
    }
}
...
function access_iopage(physicalAddress, data, byteFlag) {
    ...
    switch (physicalAddress & ~0o77) {
        case 0o17777700: // 0o17777700 - 0o17777777
            switch (physicalAddress & ~1) {
                case 0o17777776: // PSW
                ...
                case 0o17777716: // User and Super SP - note the use of odd word addresses requiring return
                    if (physicalAddress & 1) {
                        if (((CPU.PSW >> 14) & 3) == 3) { // Todo: replaced weird '(CPU.PSW >> 14) & 3 == 3' // User Mode SP
                        ...
                        if (((CPU.PSW >> 14) & 3) == 1) { // Todo: replaced weird '(CPU.PSW >> 14) && 3 == 1' // Super Mode SP
                            if (data >= 0) CPU.registerVal[6] = data;
                            result = CPU.registerVal[6];
                        } else {
                ...
                case 0o17777706: // Kernel SP & PC
                    if (physicalAddress & 1) {
                        ...
                        if (((CPU.PSW >> 14) & 3) == 0) { // Todo: replaced weird '(CPU.PSW >> 14) & 3 == 0'  // Kernel Mode
                            if (data >= 0) CPU.registerVal[6] = data;
                            result = CPU.registerVal[6];
                        } else {
            ...
            break;
        case 0o17777600: // 0o17777600 - 0o17777677 MMU user mode 3 Map
            ...
            break;
        case 0o17777500: // 0o17777500 - 0o17777577 MMR0 MMR1 MMR2 Console KW11
            switch (physicalAddress & ~1) {
                ...
                case 0o17777566: // console tty xbuf
                    ...
                    if (result >= 0 && data >= 0) {
                        ...
                        if (tty.xcsr & 0x40) {
                            tty.xcsr &= ~0x80;
                            interrupt(1, 10, 4 << 5, 0o64, tty_xcsr);
                        } else {
                            tty.xcsr |= 0x80;
                        }
                    }
                ...
                case 0o17777564: // console tty xcsr
                    ...
                    if (result >= 0 && data >= 0) {
                        if (((tty.xcsr ^ result) & 0x40)) { // IE change?
                            if (result & 0x40) {
                                if (tty.xcsr & 0x80) {
                                    interrupt(1, 0, 4 << 5, 0o64);
                                }
                            } else {
                                tty.xcsr |= 0x80;
                                interrupt(1, -1, 4 << 5, 0o64);
                            }
                ...
                case 0o17777546: // kw11.csr
                    ...
                    if (result >= 0 && data >= 0) {
                        kw11.csr = result & ~0o200;
                        if (!(kw11.csr & 0x40)) {
                            interrupt(1, -1, 6 << 5, 0o100);
                        }
                        if (kw11.init < 1) {
                            kw11.init = 1;
            ...
            break;
        case 0o17777400: // 0o17777400 - 0o17777477 rk11 controller
            result = accessRK11(physicalAddress, data, byteFlag);
            break;
        case 0o17776700: // 0o17777600 - 0o17777677 rp11 controller
            ...
            break;
        case 0o17774400: // 0o17774400 - 0o17774477 rl11 controller
            result = accessRL11(physicalAddress, data, byteFlag);
            break;
        case 0o17772500: // 0o17772500 - 0o17772577 MMR3
            switch (physicalAddress & ~1) {
            ...
            break;
        case 0o17772300: // 0o17772300 - 0o17772377 MMU kernel mode 0 Map
            idx = (physicalAddress >> 1) & 0o37;
            if (idx <= 15) {
            ...
            break;
        case 0o17772200: // 0o17772200 - 0o17772277 MMU super mode 1 Map
            idx = (physicalAddress >> 1) & 0o37;
            if (idx <= 15) {
            ...
            break;
        case 0o17772000: // 0o17772000 - 0o17772006 vt11 display
            ...
            break;
        case 0o17770300: // 0o17770300 - 0o17770377 Unibus Map
        case 0o17770200: // 0o17770200 - 0o17770277 Unibus Map
